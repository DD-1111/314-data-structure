...
public class Graph {
    ...
    private PriorityQueue<Path> prepForFindingWeightedShortestPaths(String startName) {
        currentStartVertexName = startName;
        clearAll();
        Vertex start = vertices.get(startName);
        start.weightedCostFromStartVertex = 0;
    ...
    private void buildPathFromVertex(String startVertex, PriorityQueue<Path> q) {
    ...
    public void dijkstra(String startName) {
        ...
        PriorityQueue<Path> q = prepForFindingWeightedShortestPaths(startName);
        while (!q.isEmpty()) {
            Path currentPath = q.remove();
            Vertex current = currentPath.dest;
            // if it is first visited
            if (current.scratch == 0) {
                current.scratch++;
                // go through all the adjacent edges
                for (Edge e : current.adjacent) {
                    Vertex neighborNode = e.dest;
                    // apply dijkstra, if new cost is smaller, iterate the old one
                    if (neighborNode.weightedCostFromStartVertex > current.weightedCostFromStartVertex + e.cost) {
                        Path newPath = new Path(neighborNode, currentPath.weightedCost() + e.cost);
                        neighborNode.weightedCostFromStartVertex = current.weightedCostFromStartVertex + e.cost;
    ...
    public void findAllPaths(boolean weighted) {
        allPathsFound = true;
        longest = new Path();
        for (String currentVerName : vertices.keySet()) {
            Vertex currentVer = vertices.get(currentVerName);
            currentVer.clearPathInfo();
            clearAll();
            // find shortest path
            if (weighted) {
                dijkstra(currentVerName);
            } else {
                findUnweightedShortestPath(currentVerName);
            }
            for (Vertex tempVer : vertices.values()) {
                // update vertex info
                if (!(tempVer.weightedCostFromStartVertex == INFINITY || tempVer.equals(currentVer))) {
                    currentVer.numVertexConnected++;
                    currentVer.totalUnweightedPathLength += tempVer.numEdgesFromStartVertex;
                    currentVer.totalWeightedPathLength += tempVer.weightedCostFromStartVertex;
                    // update longest path if the new path is longer than og one
                    if (weighted) {
                        if (longest.weightedCostOfPath < tempVer.weightedCostFromStartVertex) {
                            longest = getPath(tempVer.name);
                        }
                    } else {
                        if (longest.getNumEdgesInPath() < tempVer.numEdgesFromStartVertex) {
                            longest = getPath(tempVer.name);
                        }
                    }
                }
