...
public class FootballRanker {
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        double squareSum = 0;
        List<String> predictedRank = new ArrayList<>();
        for (AllPathsInfo predicted : paths) {
            String teamName = predicted.getName();
            predictedRank.add(teamName);
        }
        for (int i = 0; i < humanRanks.size(); i++) {
            String currentTeam = humanRanks.get(i);
            int difference = i - predictedRank.indexOf(currentTeam);
            squareSum += (difference * difference);
            if (showResults) {
                System.out.println(String.format("%1$-" + PADDING + "s", currentTeam) + " - actual rank: "
                        + (i + 1) + " predicted rank: " + (predictedRank.indexOf(currentTeam) + 1));
            }
        }

        double result = Math.sqrt(squareSum / humanRanks.size());
        if (showResults) {
            String resultString = String.format("%.4f", result);
            System.out.println("Root Mean Square Error: " + resultString);
        }
