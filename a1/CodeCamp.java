...
public class CodeCamp {
    ...
    */
        ...
        for(int i = 0; i < list.length;i++){
            if(list[i]!=null){
                ...
                for (int j = 0 ;j<str.length();j++){
                    if (str.substring(j,j+1).equals("A") ||str.substring(j,j+1).equals("a")||str.substring(j,j+1).equals("E")||str.substring(j,j+1).equals("e")||str.substring(j,j+1).equals("i")||str.substring(j,j+1).equals("I")||str.substring(j,j+1).equals("O")||str.substring(j,j+1).equals("o")||str.substring(j,j+1).equals("U")||str.substring(j,j+1).equals("u")){
                        count++;
                    }
                }
                if(count>max) {
    ...
    public static int sharedBirthdays(int numPeople, int numDaysInYear) {
        ...
        //CS314 STUDENTS: ADD YOUR CODE HERE
        int[] tally = new int [numDaysInYear+1];
        int[] bd = new int [numPeople];
        for (int i = 0; i < numPeople;i++){
            int currentbd = (int)(Math.random()*numDaysInYear+1);
            bd [i]=currentbd;
        }
        ...
        for (int i =0; i <tally.length;i++){
            ...
            if (tally[i]>2){
                int currentpairs = (tally[i])*(tally[i]-1)/2;
    ...
    */
        ...
        for(int y = 0;y<board.length;y++){
            for (int x=0;x<board.length;x++){
                if (board[y][x]=='q'){
                    ...
                    i =1;
                    while(y+i<board.length&&x+i<board.length){
                        if (board[y+i][x+i]=='q'){
                            return false;
                        }
                        i++;
    ...
    public static int getValueOfMostValuablePlot(int[][] city) {
        ...
        for(int y = 0;y<city.length;y++){
            for(int x=0;x<city[0].length;x++){
                for (int m=y;m<city.length;m++){
                    for(int n=x;n<city[0].length;n++){
                        int sum = 0;
                        for(int a = y;a<=m;a++) {
                            for(int b= x;b<=n;b++) {
                                sum+=city[a][b];
    ...
    public static int exmperiment1() {
        int totalpairs=0;
        for (int i = 0; i < 1000000; i++) {
            int pairs = CodeCamp.sharedBirthdays
                    (182, 365);
            totalpairs+=pairs;
        }
    ...
    public static void exmperiment2() {
        for(int j=2;j<=100;j++) {
            int numexp = 0;
            ...
            double percentage = numexp/500.0;
            System.out.println("Num people: "+j+", " +
                    "number of experiments with one or more shared birthday: "
                    +numexp+", percentage: "+percentage);
        }
    }
